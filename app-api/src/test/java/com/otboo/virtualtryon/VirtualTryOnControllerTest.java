...
class VirtualTryOnControllerTest extends IntegrationTestSupport {
    ...
    void submitsForAuthenticatedOwner() throws Exception {
        ...
        VirtualTryOnJobResponse response =
            new VirtualTryOnJobResponse(jobId, VirtualTryOnJobStatus.PENDING, null, null, false);
        given(service.submitAndRespond(eq(userId), any(VirtualTryOnRequest.class), any(MultipartFile.class)))
            .willReturn(response);
        ...
        verify(service).submitAndRespond(eq(userId), captured.capture(), eq(modelImage));
    ...
    void getsJobForAuthenticatedOwner() throws Exception {
        ...
        VirtualTryOnJobResponse response =
            new VirtualTryOnJobResponse(jobId, VirtualTryOnJobStatus.PENDING, null, null, false);
        given(service.getJobResponse(jobId, userId)).willReturn(response);

        mockMvc.perform(get("/api/fittings/{jobId}", jobId)
                .with(authentication(ownerAuthentication())))
            .andExpect(status().isOk());

        verify(service).getJobResponse(jobId, userId);
    }
